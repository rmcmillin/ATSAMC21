...
void uart_init(uint32_t baud, uint32_t fcpu){	
	...
	GCLK->PCHCTRL[SERCOM3_GCLK_ID_CORE].reg = GCLK_PCHCTRL_GEN_GCLK1 | GCLK_PCHCTRL_CHEN;
...
void printByte(uint8_t byte) {
	...
	uart_sendByte('0' + (byte / 100));                        /* Hundreds */
	uart_sendByte('0' + ((byte / 10) % 10));                      /* Tens */
	uart_sendByte('0' + (byte % 10));                             /* Ones */
}

void printWord(uint16_t word) {
	uart_sendByte('0' + (word / 10000));                 /* Ten-thousands */
	uart_sendByte('0' + ((word / 1000) % 10));               /* Thousands */
	uart_sendByte('0' + ((word / 100) % 10));                 /* Hundreds */
	uart_sendByte('0' + ((word / 10) % 10));                      /* Tens */
	uart_sendByte('0' + (word % 10));                             /* Ones */
}
